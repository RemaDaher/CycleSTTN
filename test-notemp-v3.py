...
def read_mask(mpath):
    ...
    if args.shifted:
        ...
        m_T[m!=0]=0      
    masks = [Image.fromarray(m_T*255*mult+adder)]
    return masks
...
def evaluate():
    ...
    nomask="nomask" if args.nomask else "masked"
    ...
    for video_name in os.listdir(args.frame):
        for frame in os.listdir(os.path.join(args.frame,video_name)):
            ...
            w, h=frames[0].size
            masks = read_mask(os.path.join(args.mask, video_name, frame))
                ...
                with torch.no_grad():
                    ...
                    for i in range(len(neighbor_ids)):
                        ...
                        if comp_frames[idx] is None:
                            ...
                            #Rema:
            savebasepath=os.path.join(args.output,"gen_"+args.ckptnumber.zfill(5),video_name, overlaid, shifted, nomask)
...
def main_worker():
    if args.zip:
        file1 = os.path.join(args.frame.split("JPEGImages")[0], 'files/testframes_v.npy')
        file2 = os.path.join(args.frame.split("JPEGImages")[0], 'files/testfnames_v.npy')
        file3 = os.path.join(args.frame.split("JPEGImages")[0], 'files/testmasks_v.npy')
        file4 = os.path.join(args.frame.split("JPEGImages")[0], 'files/testvideo_names.npy')
        file5 = os.path.join(args.frame.split("JPEGImages")[0], 'files/testsz.npy')
