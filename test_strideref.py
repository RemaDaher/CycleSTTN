...
parser.add_argument("-r", "--ref_length", type=int, default=10)
...
def evaluate(w, h, frames, fnames, masks, video_name):
    ...
    frameresultpath=os.path.join(savebasepath,"frameresult_Len"+str(ref_length)+"stride"+str(neighbor_stride))
    pathlib.Path(frameresultpath).mkdir(parents=True, exist_ok=True)
    writer = cv2.VideoWriter(savebasepath+"/result_Len"+str(ref_length)+"stride"+str(neighbor_stride)+".mp4", cv2.VideoWriter_fourcc(*"mp4v"), default_fps, (w, h))
    ...
    writer.release()
    print('Finish in {}'.format(savebasepath+"/result_Len"+str(ref_length)+"stride"+str(neighbor_stride)+".mp4"))
