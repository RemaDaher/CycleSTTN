...
def read_frames_mask_zip(fpath):
    ...
    masks = {}
    fnames = {}
    with open(os.path.join(fpath.split("JPEGImages")[0], 'train.json'), 'r') as f:
...
def evaluate(config, w, h, frames, fnames, masks, video_name):
    overlaid="overlaid" if args.overlaid else "notoverlaid"
    shifted="shifted" if args.shifted else "notshifted"

    # set up models 
    device = config['device'] #torch.device("cuda:"+args.gpu if torch.cuda.is_available() else "cpu")
        ...
        with torch.no_grad():
            ...
            for i in range(len(neighbor_ids)):
                ...
                if comp_frames[idx] is None:
                    ...
                    #Rema:
    savebasepath=os.path.join(args.output,"gen_"+args.ckptnumber.zfill(5),video_name, overlaid, shifted)
...
def main_worker(rank, config):
    ...
    if args.zip:
        ...
        for video_name in config['video_names']:
            frames = config['frames_v'][video_name]
            fnames = config['fnames_v'][video_name]
            masks = config['masks_v'][video_name]
            evaluate(config, w, h, frames, fnames, masks, video_name)
    else:
...
if __name__ == '__main__':
    ...
    config['init_method'] = f"tcp://{get_master_ip()}:{23455}"
    config['distributed'] = True if config['world_size'] > 1 else False

    if args.zip:
        file1 = os.path.join(args.frame.split("JPEGImages")[0], 'files/frames_v.npy')
        file2 = os.path.join(args.frame.split("JPEGImages")[0], 'files/fnames_v.npy')
        file3 = os.path.join(args.frame.split("JPEGImages")[0], 'files/masks_v.npy')
        file4 = os.path.join(args.frame.split("JPEGImages")[0], 'files/video_names.npy')
        file5 = os.path.join(args.frame.split("JPEGImages")[0], 'files/sz.npy')
