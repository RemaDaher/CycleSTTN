...
class Trainer():
    ...
    def train(self):
        ...
        while True:
            ...
            elif self.config['data_loader']['masking'] =='loaded_add': self._train_epoch_InpTargFrames_mask(pbar)
    ...
    def _train_epoch_InpTargFrames_nomask(self, pbar):
        ...
        for frames, NS_frames, empty_masks in self.train_loader:
            self.adjust_learning_rate()
            self.iteration += 1
            ...
            # discriminator adversarial loss
            real_vid_feat = self.netD(frames)
            fake_vid_feat = self.netD(pred_img.detach())
            dis_real_loss = self.adversarial_loss(real_vid_feat, True, True)
            dis_fake_loss = self.adversarial_loss(fake_vid_feat, False, True)
            dis_loss += (dis_real_loss + dis_fake_loss) / 2
            self.add_summary(
            ...
            # generator adversarial loss
            gen_vid_feat = self.netD(pred_img)
            gan_loss = self.adversarial_loss(gen_vid_feat, True, False)
            gan_loss = gan_loss * self.config['losses']['adversarial_weight']
            gen_loss += gan_loss
            self.add_summary(
            ...
            # generator l1 loss
            idt_loss = self.l1_loss(pred_img, frames) * self.config['losses']['idt_weight']
            gen_loss += idt_loss 
            self.add_summary(
                self.gen_writer, 'loss/idt_loss', idt_loss.item())
    ...
    def _train_epoch_InpTargFrames_mask(self, pbar):
        device = self.config['device']

        for frames, NS_frames, mask_trans in self.train_loader:
            self.adjust_learning_rate()
            self.iteration += 1

            frames, NS_frames, mask_trans = frames.to(device), NS_frames.to(device), mask_trans.to(device)
            b, t, c, h, w = frames.size()
            pred_img = self.netG(NS_frames, (1 - mask_trans))
            ##visualize
            # comp = (pred_img+1)/2
            # comp = comp.cpu().permute(0,2,3,1).detach().numpy()*255
            # vis_img = np.array(comp[0]).astype(np.uint8)
            ...
            mask_trans = mask_trans.view(b*t, 1, h, w)
            comp_img = frames*(mask_trans) + (1-mask_trans)*pred_img
            ...
            # generator l1 loss
            hole_loss = self.l1_loss(pred_img*mask_trans, frames*mask_trans)
            hole_loss = hole_loss / max(torch.mean(mask_trans), epsilon) * self.config['losses']['hole_weight']
            gen_loss += hole_loss 
            self.add_summary(
                self.gen_writer, 'loss/hole_loss', hole_loss.item())

            valid_loss = self.l1_loss(pred_img*(1-mask_trans), frames*(1-mask_trans))
            valid_loss = valid_loss / max(torch.mean(1-mask_trans), epsilon) * self.config['losses']['valid_weight']
            gen_loss += valid_loss 
            self.add_summary(
                self.gen_writer, 'loss/valid_loss', valid_loss.item())
